...
import { Trash2, Check, RefreshCw, ShoppingBag, MessageSquare, Mail as MailIcon, Trophy, Zap, Users, Star, CheckCircle, XCircle, Wallet, Crown, Box, AlertTriangle, Copy, Lock, LockOpen } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubBundles = onSnapshot(collection(db, 'bundles'), (snapshot) => {
      const fetched = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Bundle));
      fetched.sort((a, b) => {
    ...
    const unsubProfitRequests = onSnapshot(collection(db, 'profit_requests'), (snapshot) => {
  ...
  useEffect(() => {
    const seed = async () => {
      ...
      const existing = snaps.docs.map(d => String(d.data().dataAmount).toLowerCase().replace(/\s/g, ''));
      
      const newBundles = [
        { name: 'MTN 15GB', dataAmount: '15GB', price: 70, network: 'MTN', active: true },
        { name: 'MTN 20GB', dataAmount: '20GB', price: 100, network: 'MTN', active: true },
        { name: 'MTN 25GB', dataAmount: '25GB', price: 110, network: 'MTN', active: true },
        { name: 'MTN 30GB', dataAmount: '30GB', price: 130, network: 'MTN', active: true },
        { name: 'MTN 40GB', dataAmount: '40GB', price: 165, network: 'MTN', active: true },
        { name: 'MTN 50GB', dataAmount: '50GB', price: 250, network: 'MTN', active: true },
        { name: 'MTN 100GB', dataAmount: '100GB', price: 500, network: 'MTN', active: true }
      ];
      ...
      // AirtelTigo
      const qAirtel = query(collection(db, 'bundles'), where('network', '==', 'AirtelTigo'));
      const snapsAirtel = await getDocs(qAirtel);
      const existingAirtel = snapsAirtel.docs.map(d => String(d.data().dataAmount).toLowerCase().replace(/\s/g, ''));

      const newAirtelBundles = [
        { name: 'AirtelTigo 1GB', dataAmount: '1GB', price: 5, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 2GB', dataAmount: '2GB', price: 10, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 3GB', dataAmount: '3GB', price: 15, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 4GB', dataAmount: '4GB', price: 20, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 5GB', dataAmount: '5GB', price: 25, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 6GB', dataAmount: '6GB', price: 30, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 7GB', dataAmount: '7GB', price: 35, network: 'AirtelTigo', active: true },
        { name: 'AirtelTigo 8GB', dataAmount: '8GB', price: 40, network: 'AirtelTigo', active: true },
      ];
      for (const b of newAirtelBundles) {
        if (!existingAirtel.includes(b.dataAmount.toLowerCase().replace(/\s/g, ''))) {
           await addDoc(collection(db, 'bundles'), { ...b, createdAt: serverTimestamp() });
        }
      }
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: string, orderData?: any) => {
    try {
      ...
      const agentProfitVal = Number(orderData?.agent_profit || orderData?.profit || 0);
      const isProfitCredited = orderData?.profit_credited === true || orderData?.profitAwarded === true;
      
      if (status === 'delivered' && agentIdVal && agentProfitVal > 0 && !isProfitCredited) {
          ...
          toast.success(`Agent profit GHS ${agentProfitVal.toFixed(2)} credited to agent store! 👑`);
      }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'delivered': return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-900/50">DELIVERED</Badge>;
      case 'failed': return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 font-bold dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/50">FAILED</Badge>;
      case 'declined': return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 font-bold dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/50">DECLINED</Badge>;
      default: return <Badge variant="outline" className="dark:border-slate-800 dark:text-slate-400">{status.toUpperCase()}</Badge>;
    }
  };
  ...
  const handleMarkSeenProfitRequest = async (requestId: string) => {
    try {
      ...
      await updateDoc(doc(db, 'profit_requests', requestId), { status: 'seen' });
      toast.success("Withdrawal request marked as SEEN and balance updated! 👑");
    } catch (err: any) {
      ...
      <Tabs defaultValue="tracking" className="space-y-6">
        <div className="overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
          <TabsList className="inline-flex h-auto gap-2 p-1 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 min-w-max">
            <TabsTrigger value="tracking" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all focus-visible:ring-0 relative">
              TRACKING 👑
              {orders.filter(o => o.status === 'pending' && !o.agent_id && !o.agentId).length > 0 && (
                <span className="absolute -top-1.5 -right-1 bg-red-600 text-white text-[8px] w-4.5 h-4.5 flex items-center justify-center rounded-full font-black shadow-lg">
                  {orders.filter(o => o.status === 'pending' && !o.agent_id && !o.agentId).length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="bundles" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all focus-visible:ring-0">
              BUNDLES
            </TabsTrigger>
            <TabsTrigger value="announcement" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all focus-visible:ring-0">
              NOTIFY
            </TabsTrigger>
            <TabsTrigger value="users" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all focus-visible:ring-0">
              CUSTOMERS
            </TabsTrigger>
            <TabsTrigger value="agents" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all focus-visible:ring-0 relative">
              AGENTS HUB 👑
              {(profitRequests.filter(r => r.status === 'pending').length + orders.filter(o => o.status === 'pending' && (o.agent_id || o.agentId)).length) > 0 && (
                <span className="absolute -top-1.5 -right-1 bg-primary text-secondary text-[8px] w-4.5 h-4.5 flex items-center justify-center rounded-full font-black shadow-lg">
                  {profitRequests.filter(r => r.status === 'pending').length + orders.filter(o => o.status === 'pending' && (o.agent_id || o.agentId)).length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="complaints" className="h-9 px-4 rounded-lg font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-red-600 data-[state=active]:text-white data-[state=active]:shadow-md relative transition-all focus-visible:ring-0">
              ISSUES
              ...
              <Table>
                <TableHeader className="bg-slate-50 dark:bg-slate-900">
                  <TableRow className="border-b dark:border-slate-800">
                    <TableHead className="font-black text-[10px] uppercase tracking-wider p-4 text-slate-600 dark:text-slate-200">Customer Info</TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Service Details</TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Royal Price</TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Current Status</TableHead>
                    <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Control Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {orders.filter((o) => !o.agent_id && !o.agentId).length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="h-32 text-center text-slate-400 font-bold dark:bg-slate-950">
                        No orders yet. They show up here immediately on "BUY NOW" 👑
                      </TableCell>
                    </TableRow>
                  ) : orders.filter((o) => !o.agent_id && !o.agentId).map((order) => (
                    <TableRow key={order.id} className="hover:bg-primary/5 transition-colors group dark:border-slate-800">
                      <TableCell className="p-4">
                        <div className="flex flex-col min-w-[150px]">
                          <span className="font-bold text-slate-900 dark:text-slate-100 leading-tight">{order.customerName || order.email}</span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-tight">{order.email}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                           ...
                           {order.type === 'stream' ? (
                             ...
                             <>
                               ...
                               <div className="flex items-center gap-2">
                                 <span className="font-mono text-sm font-black text-primary bg-primary/10 px-2 py-0.5 rounded-md w-fit dark:bg-primary/20">{order.phone}</span>
                                 <Button 
                                   size="sm" 
                                   variant="outline" 
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 font-black text-secondary">
                          ...
                          <span className="dark:text-slate-300">GHS {(order.amount || 0).toFixed(2)}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                           {getStatusBadge(order.status)}
                           {order.createdAt && <span className="text-[9px] font-mono text-slate-400 dark:text-slate-500">{new Date(order.createdAt?.seconds * 1000).toLocaleString()}</span>}
                        </div>
                      </TableCell>
                      <TableCell>
                          ...
                          {order.type === 'stream' ? (
                            ...
                            <>
                              {order.status === 'pending' && (
                                <>
                                  ...
                                  <Button 
                                    ...
                                    onClick={() => handleUpdateOrderStatus(order.id, 'declined', order)}
                                  >
                              ...
                              <Button 
                                ...
                                title="Delete Order"
                                onClick={() => {
                                  if(window.confirm("Royal Decree: Delete this order permanently? 👑")) {
                                    deleteDoc(doc(db, 'orders', order.id));
                                    toast.success("Order removed from the collection! 👑");
                                  }
                                }}
                              >
                                <Trash2 className="w-4 h-4" />
                              </Button>
                              {order.status === 'delivered' && order.bundle === 'AGENT ACCESS UNLOCK' && order.userId && (
                                <Button
                                  size="sm"
                                  variant="destructive"
                                  className="h-9 px-4 rounded-xl font-black uppercase text-[10px] shadow-sm flex items-center gap-2"
                                  onClick={() => handleRelockStore(order.userId || '', order.id)}
                                >
                                  <Lock className="w-3 h-3" />
                                  Relock Store
                                </Button>
                              )}
                              {order.status === 'declined' && order.bundle === 'AGENT ACCESS UNLOCK' && order.userId && (
                                <Button
                                  size="sm"
                                  className="h-9 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-[10px] shadow-sm flex items-center gap-2"
                                  onClick={() => handleUnlockStore(order.userId || '', order.id)}
                                >
        ...
        <TabsContent value="announcement" className="mt-0 outline-none">
           <Card className="rounded-3xl border-2 bg-white dark:bg-slate-950 dark:border-slate-800 overflow-hidden shadow-sm">
             <CardHeader className="p-8 border-b dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
               <CardTitle className="text-2xl font-black text-slate-900 dark:text-white">Discount Settings 👑</CardTitle>
             </CardHeader>
             <CardContent className="p-8 pt-6">
               <form onSubmit={handleUpdateAnnouncement} className="space-y-6 max-w-2xl">
                  <div className="space-y-2">
                    <Label className="font-bold underline underline-offset-4 text-slate-700 dark:text-slate-300">Top Bar Text</Label>
                    <Input 
                  ...
                  <div className="flex items-center gap-6 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border-2 dark:border-slate-800">
                    <div className="flex-1">
                      <h4 className="font-black text-sm uppercase text-slate-900 dark:text-slate-100">Active Status</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">When active, the top bar appears and dynamic pricing is applied.</p>
                    </div>
                    <button 
                      type="button"
           ...
           <Card className="rounded-3xl border-2 bg-white dark:bg-slate-950 dark:border-slate-800 shadow-sm overflow-hidden">
             ...
             <CardContent className="p-0 overflow-x-auto">
               <Table>
                 <TableHeader className="bg-slate-50 dark:bg-slate-900">
                    <TableRow className="border-b dark:border-slate-800">
                      <TableHead className="p-6 font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Royal Customer</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Email Address</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-right p-6 text-slate-600 dark:text-slate-200">Balance</TableHead>
                    </TableRow>
                 </TableHeader>
                 <TableBody>
                   {users.map(u => (
                     <TableRow key={u.id} className="hover:bg-slate-50/50 transition-colors dark:border-slate-800">
                       <TableCell className="p-6 min-w-[150px]">
                         <span className="font-bold text-slate-900 dark:text-slate-100">{u.fullName}</span>
                       </TableCell>
                       <TableCell className="font-mono text-xs text-slate-500 dark:text-slate-400 min-w-[200px]">{u.email}</TableCell>
                       <TableCell className="text-right p-6 font-black text-primary">GHS {u.walletBalance?.toFixed(2) || '0.00'}</TableCell>
                     </TableRow>
                   ))}
                 </TableBody>
               </Table>
              ...
              <CardContent className="p-6">
                ...
                <Tabs defaultValue="requests" className="space-y-6">
                  <TabsList className="bg-slate-100 dark:bg-slate-900 p-1 rounded-xl flex w-full max-w-lg h-auto mb-6">
                    <TabsTrigger value="requests" className="flex-1 rounded-lg py-2 font-black text-[10px] uppercase relative flex items-center justify-center gap-1.5 transition-all">
                      <span>Profit Requests</span>
                      {profitRequests.filter(r => r.status === 'pending').length > 0 && (
                    ...
                    <TabsTrigger value="list" className="flex-1 rounded-lg py-2 font-black text-[10px] uppercase">List of Agents</TabsTrigger>
                    <TabsTrigger value="orders" className="flex-1 rounded-lg py-2 font-black text-[10px] uppercase relative flex items-center justify-center gap-1.5 transition-all">
                      ...
                      <Table>
                        <TableHeader className="bg-slate-50 dark:bg-slate-900">
                          <TableRow className="border-b">
                            <TableHead className="p-4 font-bold text-[10px] uppercase">Agent Details</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase">MoMo Channel</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase">Request Amount</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase">Submitted Date</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase text-right p-4">Actions</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          ...
                          ) : [...profitRequests].sort((a,b) => {
                            if (a.status === 'pending' && b.status !== 'pending') return -1;
                            if (a.status !== 'pending' && b.status === 'pending') return 1;
                            return 0;
                          }).map(req => (
                            <TableRow key={req.id} className="hover:bg-slate-50/50 dark:border-slate-800">
                              <TableCell className="p-4">
                                <span className="font-extrabold text-foreground">{req.agent_name}</span>
                              </TableCell>
                              <TableCell>
                                <div className="text-xs">
                                  <p className="font-bold">{req.momo_name}</p>
                                  <div className="flex items-center space-x-2">
                                    <p className="font-mono text-slate-500">{req.momo_number}</p>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      title="Copy Number"
                                      ...
                                      onClick={() => {
                                        navigator.clipboard.writeText(req.momo_number || '');
                              ...
                              <TableCell className="font-black text-primary">GHS {Number(req.withdrawal_amount).toFixed(2)}</TableCell>
                              <TableCell className="text-xs text-slate-500 font-medium">
                                {req.created_at ? new Date(req.created_at.seconds * 1000).toLocaleString() : 'Just now'}
                              </TableCell>
                              <TableCell className="text-right p-4">
                                {req.status === 'pending' ? (
                                  <Button
                                    ...
                                    onClick={() => handleMarkSeenProfitRequest(req.id)}
                                  >
                  ...
                  <TabsContent value="list" className="outline-none">
                    <div className="overflow-x-auto border rounded-2xl">
                      <Table>
                        <TableHeader className="bg-slate-50 dark:bg-slate-900">
                          <TableRow className="border-b">
                            <TableHead className="p-4 font-bold text-[10px] uppercase">Agent / Slug</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase">MoMo Withdrawal Link</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase text-right p-4">Available Profit Balance</TableHead>
                            <TableHead className="font-bold text-[10px] uppercase text-right p-4">Actions</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {agents.length === 0 ? (
                            ...
                            <TableRow key={ag.id} className="hover:bg-slate-50/50 dark:border-slate-800">
                              <TableCell className="p-4">
                                <div className="flex flex-col">
                                  <span className="font-black text-foreground">{ag.agent_name}</span>
                                  <span className="text-[10px] text-primary font-bold lowercase">/store/{ag.agent_slug}</span>
                                </div>
                              </TableCell>
                              <TableCell>
                                <div className="text-xs">
                                  <p className="font-semibold">{ag.momo_name}</p>
                                  <p className="font-mono text-slate-500">{ag.momo_number}</p>
                                </div>
                              </TableCell>
                              <TableCell className="text-right p-4 font-black text-green-600">GHS {Number(ag.profit_balance || 0).toFixed(2)}</TableCell>
                              <TableCell className="text-right p-4">
                                <Button 
                                  size="sm" 
                                  variant="destructive"
                  ...
                  <TabsContent value="orders" className="outline-none">
                    <div className="overflow-x-auto border rounded-2xl">
                       <Table>
                         <TableHeader className="bg-slate-50 dark:bg-slate-900">
                           <TableRow className="border-b">
                             <TableHead className="p-4 font-bold text-[10px] uppercase">Order ID / Date</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase">Package & Number</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase">Agent Origin</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase">Wholesale Price</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase">Agent Price</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase">Agent Profit</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase text-right p-4">Status</TableHead>
                             <TableHead className="font-bold text-[10px] uppercase text-right p-4">Control Actions</TableHead>
                           </TableRow>
                         </TableHeader>
                         <TableBody>
                           {orders.filter(o => o.agent_id || o.agentId).length === 0 ? (
                             <TableRow>
                               <TableCell colSpan={8} className="h-32 text-center text-slate-400 font-bold">
                                 No agent sales generated yet.
                               </TableCell>
                             </TableRow>
                           ) : orders.filter(o => o.agent_id || o.agentId).map(o => (
                             <TableRow key={o.id} className="hover:bg-slate-50/50 dark:border-slate-800">
                               <TableCell className="p-4">
                                 <span className="font-mono text-xs font-bold uppercase">{o.id.slice(-8)}</span>
                                 ...
                                 <div className="text-xs">
                                   <div className="flex items-center space-x-2">
                                     ...
                                     <Button
                                       ...
                                       className="h-5 w-5 p-0 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
                                       onClick={() => {
                                         navigator.clipboard.writeText(o.bundle || '');
                               ...
                               <TableCell>
                                 <span className="font-extrabold text-foreground">{o.agent_name || o.agentName || 'N/A'}</span>
                               </TableCell>
                               <TableCell className="font-mono text-xs">GHS {Number(o.wholesale_price || o.wholesalePrice || 0).toFixed(2)}</TableCell>
                               <TableCell className="font-mono text-xs font-bold">GHS {Number(o.agent_price || o.agentPrice || 0).toFixed(2)}</TableCell>
                               <TableCell className="font-mono text-xs font-black text-green-600">GHS {Number(o.agent_profit || o.profit || 0).toFixed(2)}</TableCell>
                               <TableCell className="text-right p-4">
                                   ...
                                   {o.status === 'pending' && (
                                     <>
                                       <Button 
                                         ...
                                         onClick={() => handleUpdateOrderStatus(o.id, 'processing', o)}
                                       >
                                         <Check className="w-3 h-3" />
                                         Accept 👑
                                       </Button>
                                       <Button 
                                         ...
                                         onClick={() => handleUpdateOrderStatus(o.id, 'declined', o)}
                                       >
                                         <XCircle className="w-3 h-3" />
                                         Decline
                                       </Button>
                                   ...
                                   {o.status === 'processing' && (
                                     <>
                                       <Button 
                                         ...
                                         onClick={() => handleUpdateOrderStatus(o.id, 'delivered', o)}
                                       >
                                         <CheckCircle className="w-3 h-3" />
                                         Deliver 👑
                                       </Button>
           ...
           <Card className="rounded-3xl border-2 bg-white dark:bg-slate-950 dark:border-slate-800 shadow-sm overflow-hidden">
             ...
             <CardContent className="p-0 overflow-x-auto">
               <Table>
                 <TableHeader className="bg-slate-50 dark:bg-slate-900">
                    <TableRow className="border-b dark:border-slate-800">
                      <TableHead className="p-6 font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Customer Info</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Issue Description</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-slate-600 dark:text-slate-200">Status</TableHead>
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-right p-6 text-slate-600 dark:text-slate-200">Actions</TableHead>
                    </TableRow>
                 </TableHeader>
                 <TableBody>
                   {complaints.length === 0 ? (
                     <TableRow>
                       <TableCell colSpan={4} className="h-48 text-center text-slate-400 font-bold dark:bg-slate-950">
                     ...
                     <TableRow key={c.id} className="hover:bg-red-50/10 transition-colors dark:border-slate-800">
                       <TableCell className="p-6 min-w-[150px]">
                         <div className="flex flex-col">
                           <span className="font-bold text-slate-900 dark:text-slate-100">{c.userEmail}</span>
                           {c.subject && (c.subject.includes('[Agent Report]') || c.subject.includes('--- AGENT REPORT ---')) && (
                       ...
                       <TableCell className="py-4 min-w-[200px]">
                         {c.subject && (c.subject.includes('[Agent Report]') || c.message.includes('--- AGENT REPORT ---')) ? (
                           ...
                           <p className="text-xs font-medium max-w-sm line-clamp-2 text-slate-600 dark:text-slate-400" title={c.message}>{c.message}</p>
                         )}
                       </TableCell>
                       <TableCell>
                         <Badge 
