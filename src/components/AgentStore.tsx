...
import { doc, getDoc, setDoc, updateDoc, collection, query, where, or, getDocs, onSnapshot, serverTimestamp, addDoc, deleteDoc } from 'firebase/firestore';
...
[Please type your reason here...]`;
  ...
  const acceptedOrdersForTracking = orders;
          ...
          <TabsContent value="orders_list" className="outline-none">
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                ...
                <Card className="rounded-[1.5rem] border-2 shadow-sm overflow-hidden bg-card">
                   <CardContent className="p-5 flex flex-col justify-between h-full gap-2">
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Successful & Paid Volume</span>
                     <h4 className="text-2xl sm:text-3xl font-black text-green-600">
                       GHS {acceptedOrdersForTracking.filter(o => o.status === 'delivered' || o.status === 'completed' || o.status === 'paid' || o.status === 'success' || o.status === 'successful' || o.paymentStatus === 'success').reduce((sum, o) => sum + Number(o.agent_price || o.agentPrice || o.amount || 0), 0).toFixed(2)}
                     </h4>
                   </CardContent>
                </Card>
                ...
                <Card className="rounded-[1.5rem] border-2 shadow-sm overflow-hidden bg-card">
                   <CardContent className="p-5 flex flex-col justify-between h-full gap-2">
                     ...
                     <h4 className="text-2xl sm:text-3xl font-black text-primary">
                       GHS {acceptedOrdersForTracking.filter(o => o.status === 'delivered' || o.status === 'completed' || o.status === 'paid' || o.status === 'success' || o.status === 'successful' || o.paymentStatus === 'success').reduce((sum, o) => sum + Number(o.agent_profit || o.profit || 0), 0).toFixed(2)}
                     </h4>
                   </CardContent>
                </Card>
                ...
                <Card className="rounded-[1.5rem] border-2 shadow-sm overflow-hidden bg-card">
                   <CardContent className="p-5 flex flex-col justify-between h-full gap-2">
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Pending / Processing</span>
                     <h4 className="text-2xl sm:text-3xl font-black text-yellow-600">
                       {acceptedOrdersForTracking.filter(o => o.status === 'processing' || o.status === 'pending' || o.status === 'pending_verification').length} <span className="text-xs text-slate-400 font-bold">orders</span>
                     </h4>
                   </CardContent>
                </Card>
              </div>
