...
import { GraduationCap, ArrowRight, Sparkles, CheckCircle2, AlertCircle } from 'lucide-react';
      ...
      >
        ...
        <div className="flex items-center gap-3.5 sm:gap-4 w-full sm:w-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
              ...
              {isOutOfStock ? (
                <span className="text-[10px] text-red-400 font-bold flex items-center gap-0.5 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">
        ...
        <div className="w-full sm:w-auto shrink-0 flex items-center justify-end">
          <Button
            ...
            onClick={handleClick}
            className={`w-full sm:w-auto h-11 px-5 sm:px-6 rounded-xl font-black text-xs sm:text-sm flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer ${
              isOutOfStock
                ? 'bg-slate-800 text-amber-300 border border-amber-500/30 hover:bg-slate-700'
                : 'bg-gradient-to-r from-amber-400 via-amber-300 to-yellow-500 text-slate-950 hover:brightness-110 shadow-[0_2px_12px_rgba(245,158,11,0.3)] border border-amber-300/60'
            }`}
          >
            <span>{isOutOfStock ? 'View Checker (Out of Stock)' : 'Buy Results Checker'}</span>
